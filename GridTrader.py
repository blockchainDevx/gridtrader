...
class GridTrader():
    ...
    def __del__(self):
        ...
        while i< len(self.order_list):
            order_info=self.order_list[i]
            try:
                order=self.exchange.cancel_order(order[0],self.api_symbol)
        ...
        if self.has_qty >0:
            try:
                order=self.exchange.create_order(self.api_symbol,self.type[0],self.side[1],self.has_qty)
                qty=self.has_qty
                self.log(f'程序结束,将所有持仓已市价卖掉:{qty}')
            except Exception as e:
                strr = str(e)
                self.log(f'程序结束,将所有持仓卖掉出错,原因:{strr}')
    ...
    def init(self):
        ...
        # 交易所连接
        if(self.api_exchange.lower()=='okex'):
        ...
        while last < self.grid_list[0][0] or last>self.grid_list[-1][0]:
            self.log('价格没在网格区间')
            last=self.exchange.fetch_ticker(self.api_symbol)['last']
        ...
        if qty_depth > qty:
            #深度足够
            try:
                order= self.exchange.create_order(self.api_symbol,self.type[0],self.side[0],qty)
                while True:
                    try:
                        order_ret= self.exchange.fetch_order(order['id'],self.api_symbol)
                        if order_ret['status']=='closed':
                            #开单手数减去手续费扣得币
                            fee = float(order_ret['fee']['cost'])
                            self.has_qty=float(order_ret['filled']-fee)
                            self.log(f'开单成功,手数:{qty},手续费扣的币:{fee},已持有币:{self.has_qty}')
                            return True
                    except:
                        pass
                    time.sleep(0.5)
            ...
            #深度不够,当前价挂单
            try:
                order=self.exchange.create_order(self.api_symbol,self.type[1],self.side[0],qty,last)
                while True:
                    try:
                        order_ret=self.exchange.fetch_order(order['id'],self.api_symbol)
                        if order_ret['status']=='closed':
        ...
        # grid_qty_low,self.grid_qty_up,grid_price_low,grid_price_up = self.get_current_grid_data(tick['last'])
        # self.log(f'按照最新价格算出,价格上面有:{self.grid_qty_up}格,下面有:{grid_qty_low},格子上沿价格为:{grid_price_up},格子下沿价格为:{grid_price_low}')

        #此时价格所在网格数(从网格0开始)
        # self.current_grid_no=grid_qty_low

        #此时入场买入的数量
        # qty=((self.fund_per_grid*(1+self.grid_taker)/grid_price_up)*(1/(1+self.ratio_per_grid))**self.grid_qty_up-
        # (self.fund_per_grid*(1+self.grid_taker)/grid_price_up))/((1/(1+self.ratio_per_grid))-1)

        # qty=((self.fund_per_grid/grid_price_up)*(1/(1+self.ratio_per_grid))**self.grid_qty_up-
        # (self.fund_per_grid/grid_price_up))/((1/(1+self.ratio_per_grid))-1)
        # qty=round(qty,self.api_qtyreserve)
        # qty1=round(qty*(1+self.grid_taker),self.api_pricereserve)
        # self.log(f'此时需要买入的手数为:{qty1}')
        ...
        # max_price=tick['last']*(1+self.grid_slip)
        ...
        if(qty_depth>qty):
            #深度足够,市价开单
            order=self.create_order(self.side[0],qty)
            if(order==None):            
                return False
            self.log(f'开单检查深度足够,开市价单:{order}')
            while True:
                info=self.exchange.fetch_order(order['id'],self.api_symbol)
                if(info['status']=='closed'):
                    self.log(f'开单已成交,开单状态{info}')
                    _fee=info['fee']
                    self.has_qty=qty - float(info['fee']['cost'])
                    return True
                else:
                    time.sleep(0.5)
        else:
            #深度不够,开限价单,然后等待完全成交之后开始网格
            order=self.create_order('buy',qty,tick['last'])
            if order==None:
                return False
            self.log(f'开单检查深度不够,开挂单,{order}')
            while True:
                info=self.exchange.fetch_order(order['id'],self.api_symbol)
                if(info['status']=='closed'):
                    self.log(f'开单已成交,开单信息{info}')
                    self.has_qty=qty- float(info['fee']['cost'])
                    return True
                else:
                    #每隔0.5s检查一次单子的状态
    ...
    def get_current_grid_no(self,last):
        ...
        #价格跌破网格
        if self.grid_list[0][0] >= last:
            return -1

        #价格涨破网格
        if self.grid_list[-1][0] <= last:
            return -1

        for index in range(len(self.grid_list)-1):
            if self.grid_list[index +1][0] >last and self.grid_list[index][0] <last:
                return index
    ...
    def create_grid(self):
        ...
        for index in range(len(self.grid_list)):
            if index <= self.current_gridno:
                #比当前价所在的网格低的开买单
                try:
                    price= self.grid_list[index][0]
                    qty=self.grid_list[index][1]
                    order = self.exchange.create_order(self.api_symbol,self.type[1],self.side[0],qty,price)
                    self.order_list.append([order['id'],self.side[0], qty,price])
                    self.log(f'网格创建成功,网格编号为{index},网格下沿价格为{price},挂单手数{qty}')
                except:
                    self.log(f'网格挂单创建失败,网格编号为{index},方向为{self.side[0]},网格下沿价格为{price},需要买的数量为{qty}')
                    pass
                ...
                try:
                    ...
                    has_qty=has_qty-qty
                    self.order_list.append([order['id'],self.side[1], qty,price])
                    self.log(f'网格创建成功,网格编号为{index},方向为{self.side[1]},网格下沿价格为{price},挂单手数{qty}')
                except:
                    self.log(f'网格挂单创建失败,网格编号为{index},网格下沿价格为{price},需要卖的数量为{qty}')
                    pass
            time.sleep(0.2)
        ...
        if current_gridno>self.current_gridno:
            ...
            for index in range(self.current_gridno+1,current_gridno+1):
                try:
                    ...
                    order= self.exchange.create_order(self.api_symbol,self.type[1],self.side[0], qty,price)
                    self.log(f'网格突破,挂买单,委托信息为{order}')
                    self.order_list.append([order['id'],self.side[0],qty,price])
                except Exception as e:
                    strr=str(e)
                    self.log(f'挂单失败:原因为{strr}')
                    pass
    ...
    #价格网格判断
    # def price_check(self,last):
    #     _,grid_qty_up,grid_price_low,grid_price_up = self.get_current_grid_data(last)
    #     if grid_qty_up>self.grid_qty_up:
    #         self.grid_qty_up=grid_qty_up
    #         #当前价格的上面格子数大于前一价格的上面格子数,表示价格在下降,此时挂买单,数量为每格金额*(1-手续费)/下沿价格
    #         qty=round(self.fund_per_grid*(1+self.grid_maker)/grid_price_low,self.api_qtyreserve)
    #         order=self.create_order(self.side[0],qty,round(grid_price_low,self.api_pricereserve))
    #         if order== None:
    #             #委托出错
    #             self.log(f'网格向上突破,突破网格价格为{grid_price_low},挂单失败')
    #             pass
    #         #挂单记录下来
    #         self.order_list.append([order['id'],self.side[0],qty])
    #         self.log(f'网格向上突破,突破网格价格为{grid_price_low},挂单成功:{order}')
    #     elif grid_qty_up<self.grid_qty_up:
    #         self.grid_qty_up=grid_qty_up
    #         #当前价格的上面格子数小于前一价格的上面格子数,表示价格在上升,此时挂卖单,数量为每格金额*(1-手续费)/上沿价格
    #         qty=round(self.fund_per_grid*(1+self.grid_maker)/grid_price_up ,self.api_qtyreserve)

    #         #当计算的手数大于已有手数时,挂单手数为已有手数
    #         if qty>self.has_qty: 
    #             qty=self.has_qty
    #         order=self.create_order(self.side[1],qty,round(grid_price_up,self.api_pricereserve))
    #         if order==None:
    #             #委托出错
    #             self.log(f'网格向上突破,突破网格价格为{grid_price_up},挂单失败')
    #             return
    #         self.log(f'网格向上突破,突破网格价格为{grid_price_up},挂单成功:{order}')
    #         self.order_list.append([order['id'],self.side[1],qty])
    ...
    def stop_grid(self):
        ...
        while i<len(self.order_list):
            ...
            try:
                self.exchange.cancel_order(order[0])
                self.order_list.pop(i)
            except:
                self.log(f'取消挂单失败,原因:{strr}')
                pass
        
        #将所持有手数全在止损价挂卖单
        if self.has_qty >0:
            try:
                order=self.exchange.create_order(self.api_symbol,self.type[1],
                self.side[1],self.has_qty,self.grid_stop)
            except Exception as e:
                strr=str(e)
                self.log(f'止损卖挂单失败,原因:{strr}')
    ...
    def update_order(self,last):
        ...
        while i<len(self.order_list):
            order=self.order_list[i]
            if order[1]==self.side[0] and order[3] >=last:
                #委托单位买,且买价大于等于最新价,表示挂单已成交,更新委托信息
                try:
                    info = self.exchange.fetch_order(order[0],self.api_symbol)
                    if info['status']=='closed':
                        self.has_qty=self.has_qty+(float(info['filled']-float(info['fee']['cost'])))
                        self.log(f'网格挂单成交,委托为:{info}')
                        self.order_list.pop(i)
                except: 
                    pass
            elif order[1]==self.side[1] and order[3] <= last:
                #委托单为卖,且卖价小于等于最新价,表示挂单或已成交,更新委托信息
                try:
                    info = self.exchange.fetch_order(order[0],self.api_symbol)
                    if info['status']=='closed':
                        self.has_qty=self.has_qty-order[2]
                        self.log(f'网格挂单成交,委托为:{info}')
                        self.order_list.pop(i)
                except:
                    pass
    ...
    # def get_current_grid_data(self,last):
    #     #根据最新价格计算出此时的价格所在的格子的上沿价格,下沿价格,上面有多少格,下面有多少格
    #     #此时的下面格数
    #     grid_qty_low=int(math.log10(last/self.grid_lowbound)/(math.log10(1+self.ratio_per_grid)))
        
    #     #此时上面格数
    #     grid_qty_up=self.grid_gridqty-grid_qty_low-1

    #     #此时价格所在格子下沿价格
    #     grid_price_low=self.grid_lowbound*(1+self.ratio_per_grid)**grid_qty_low
    #     grid_price_low=round(grid_price_low,self.api_pricereserve)

    #     #此时价格所在格子上沿价格
    #     grid_price_up=grid_price_low*(1+self.ratio_per_grid)
    #     grid_price_up=round(grid_price_up,self.api_pricereserve)
    #     return grid_qty_low,grid_qty_up,grid_price_low,grid_price_up


    #下单封装,参数为方向,数量,价格
    # def create_order(self,side,qty,price=None):
    #     type=self.type[0]
    #     if(price!=None):
    #         type=self.type[1]
    #     self.log(f'下单:,方向{side},数量:{qty},价格:{price},类型:{type}')
    #     try:
    #         return self.exchange.create_order(self.api_symbol, 
    #         type,
    #         side,
    #         qty,
    #         price if price != None else None)
    #     except Exception as e:
    #         estr=str(e)
    #         self.log(f"下单失败{estr}")
    #         return None
    
    #根据网格设置计算出所有的网格价格,保留小数点后6位
    def calc_grid_list(self):
        strr='网格价格'
        for i in range(0,self.grid_gridqty+1):
            ...
            self.grid_list.append([price,qty,qty2])
        self.log(f'{strr}')
    ...
    def read_config(self,file='config.ini'):
        ...
        #api配置
        self.api_exchange=cfg.get('api','exchange')
        if self.api_exchange.lower() != 'okex':
            raise Exception('交易所设置错误,目前只支持OKEX',self.api_exchange)
    ...
    def log(self,msg,withTime=True):
        timestamp=datetime.now(pytz.timezone('UTC')).strftime("%b %d %Y %H:%M:%S, ")
        try:
