...
class HalfGridVariantTrader(IGridTrader):
    ...
    def parms_check(json_data):
        ...
        if exchange.lower() != OKEX and \
           exchange.lower() != FTX and \
           exchange.lower() !=BINANCE and \
           exchange.lower()!=GATE:
    ...
    def open_order(self,last,qty):
        ...
        order,err_msg=self.trade_hd.CreateOrder(self.api_symbol, type,BUY,qty,price)
        if order==None:
            return f'开仓失败,{err_msg}',None
        return 'OK',order['id']
    ...
    def condition_create_order(self,symbol,type,side,qty,price,last):
        ...
        order,err_msg=self.trade_hd.CreateOrder(symbol,type,side,qty,price)
        return order
        pass
